...
export function Input({ label, error, type, className, ...props }: InputProps) {
  ...
  return (
    <div className="flex flex-col gap-[5px]">
      ...
      <div className="relative">
        <input
          ...
          className={cn(
            ...
            'px-[12px] py-[8px] outline-none transition-all duration-150',
            'stone-inset',
            'placeholder:text-diablo-text-dim placeholder:font-normal',
            'focus:border-diablo-border-light focus:shadow-diablo-gold',
            error && 'border-diablo-red',
            isPassword && 'pr-[44px]',
            className,
          )}
          {...props}
        ...
        {isPassword && (
          <button
            ...
            className={cn(
              ...
              'border-l border-diablo-border bg-[#0e0a06]',
              'transition-colors duration-150',
              visible ? 'text-diablo-gold' : 'text-diablo-text-dim hover:text-diablo-gold',
            )}
          >
      ...
      {error && (
        <span className="text-[11px] tracking-[1px] text-[#cc4040] font-medium font-diablo">
          {error}
        </span>
      )}
    </div>
